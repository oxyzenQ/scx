...
#define WOLF_CGRP_NAME_MAX	16

static const char wolf_cgrp_latency[WOLF_CGRP_NAME_MAX] = "wolf.latency";
static const char wolf_cgrp_bulk[WOLF_CGRP_NAME_MAX] = "wolf.bulk";
...
const volatile u64 lane_min_dwell_ns = 200ULL * NSEC_PER_MSEC;
const volatile u64 lane_lat_nvcsw_hi = 16ULL;
const volatile u64 lane_lat_nvcsw_lo = 8ULL;
const volatile u64 lane_bulk_exec_ns = 16ULL * NSEC_PER_MSEC;
...
volatile u64 lane_switch_total, latency_starvation_events, bulk_run_ns_total;
 ...
 */
private(WOLF) struct bpf_cpumask __kptr *primary_cpumask;
...
static inline u64 cpu_lane_dsq(u8 lane, s32 cpu)
{
	return (lane == WOLF_LANE_BULK ? DSQ_FLAG_BULK : 0) | (u64)cpu;
}

static inline u64 node_lane_dsq(u8 lane, int node)
{
	return (lane == WOLF_LANE_BULK ? DSQ_FLAG_BULK : 0) | DSQ_FLAG_NODE | node;
}

static __always_inline bool str_eq_fixed(const char *a, const char *b)
{
	int i;

	#pragma clang loop unroll(full)
	for (i = 0; i < WOLF_CGRP_NAME_MAX; i++) {
		char ca = a[i];
		char cb = b[i];
		if (ca != cb)
			return false;
...
static __always_inline bool cgrp_matches_name(struct cgroup *cgrp, const char *target)
{
	char name[WOLF_CGRP_NAME_MAX];
	int ret;

	ret = bpf_probe_read_kernel_str(name, sizeof(name), BPF_CORE_READ(cgrp, kn, name));
	if (ret < 0)
		return false;

	return str_eq_fixed(name, target);
}

static __always_inline s32 cgrp_lane_override_from_id(u64 cgrp_id)
{
	struct cgroup *cgrp;
	u32 level, max_level;
	s32 lane = -1;
	...
	if (cgrp_matches_name(cgrp, wolf_cgrp_latency)) {
		lane = WOLF_LANE_LATENCY;
		goto out;
	}
	if (cgrp_matches_name(cgrp, wolf_cgrp_bulk)) {
		lane = WOLF_LANE_BULK;
		goto out;
	}
	...
	bpf_for(i, 1, max_level + 1) {
		const char *kname = BPF_CORE_READ(cgrp, ancestors[i], kn, name);
		char name[WOLF_CGRP_NAME_MAX];
		int ret = bpf_probe_read_kernel_str(name, sizeof(name), kname);
		if (ret < 0)
			continue;
		if (str_eq_fixed(name, wolf_cgrp_latency)) {
			lane = WOLF_LANE_LATENCY;
			break;
		}
		if (str_eq_fixed(name, wolf_cgrp_bulk)) {
			lane = WOLF_LANE_BULK;
			break;
		}
	}
...
static __always_inline bool task_is_rt_hint(const struct task_struct *p)
{
	return p->policy == SCHED_FIFO || p->policy == SCHED_RR;
}

static __always_inline u8 behavior_lane_candidate(const struct task_struct *p, const struct task_ctx *tctx)
{
	u64 hi = lane_lat_nvcsw_hi;
	u64 lo = lane_lat_nvcsw_lo;
	bool rt_hint = task_is_rt_hint(p);
	...
	if (tctx->lane == WOLF_LANE_LATENCY) {
		if (tctx->exec_runtime >= lane_bulk_exec_ns && tctx->avg_nvcsw <= lo)
			return WOLF_LANE_BULK;
		return WOLF_LANE_LATENCY;
	}

	if (tctx->avg_nvcsw >= hi)
		return WOLF_LANE_LATENCY;
	return WOLF_LANE_BULK;
}

static __always_inline u8 update_task_lane(struct task_struct *p, struct task_ctx *tctx)
{
	u64 now = scx_bpf_now();
	u8 cand;

	if (tctx->lane_override >= 0) {
		if (tctx->lane != (u8)tctx->lane_override) {
			tctx->lane = (u8)tctx->lane_override;
			tctx->lane_last_change = now;
			__sync_fetch_and_add(&lane_switch_total, 1);
		}
		return tctx->lane;
	}

	cand = behavior_lane_candidate(p, tctx);
	if (cand != tctx->lane && time_delta(now, tctx->lane_last_change) >= (s64)lane_min_dwell_ns) {
		tctx->lane = cand;
		tctx->lane_last_change = now;
		__sync_fetch_and_add(&lane_switch_total, 1);
	}
...
static inline u64 nr_tasks_waiting(s32 cpu)
{
	...
	return scx_bpf_dsq_nr_queued(SCX_DSQ_LOCAL_ON | cpu) +
	       scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_LATENCY, cpu)) +
	       scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_BULK, cpu)) +
	       scx_bpf_dsq_nr_queued(node_lane_dsq(WOLF_LANE_LATENCY, node)) +
	       scx_bpf_dsq_nr_queued(node_lane_dsq(WOLF_LANE_BULK, node));
}
			 ...
			 s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	...
	if (is_wake_sync(current, prev_cpu, this_cpu, wake_flags)) {
		...
		if ((share_llc || current->pid == tctx->waker_pid) &&
		    ...
		    !scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_LATENCY, this_cpu)) &&
		    !scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_BULK, this_cpu)) &&
				...
				s32 prev_cpu, u64 enq_flags)
{
	...
	if (direct_dispatch && !is_cpu_busy(cpu)) {
		scx_bpf_dsq_insert_vtime(p, cpu_lane_dsq(tctx->lane, cpu),
					 task_slice(p, cpu), p->scx.dsq_vtime, enq_flags);
	...
	if (can_direct_dispatch(cpu)) {
		scx_bpf_dsq_insert_vtime(p, cpu_lane_dsq(tctx->lane, cpu),
					 task_slice(p, cpu), p->scx.dsq_vtime, 0);
		__sync_fetch_and_add(&nr_direct_dispatches, 1);
		dispatched = true;
	}
...
void BPF_STRUCT_OPS(flash_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (!scx_bpf_task_running(p) && can_enqueue_to_cpu(p, prev_cpu)) {
		scx_bpf_dsq_insert_vtime(p, cpu_lane_dsq(tctx->lane, prev_cpu),
	...
	scx_bpf_dsq_insert_vtime(p, node_lane_dsq(tctx->lane, node),
				 task_slice(p, prev_cpu), p->scx.dsq_vtime, enq_flags);
	__sync_fetch_and_add(&nr_shared_dispatches, 1);
...
void BPF_STRUCT_OPS(flash_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	// Invariants:
	// 1. Latency lane always dispatched first
	// 2. Bulk lane never starves latency
	// 3. Lane switches are hysteretic + dwell-limited

	/*
	 * Try to consume a task from the per-CPU DSQ.
	 */
	if (scx_bpf_dsq_move_to_local(cpu_lane_dsq(WOLF_LANE_LATENCY, cpu)))
		return;
	if (scx_bpf_dsq_move_to_local(node_lane_dsq(WOLF_LANE_LATENCY, node)))
		return;
	if (scx_bpf_dsq_move_to_local(cpu_lane_dsq(WOLF_LANE_BULK, cpu))) {
		if (scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_LATENCY, cpu)) ||
		    scx_bpf_dsq_nr_queued(node_lane_dsq(WOLF_LANE_LATENCY, node)))
			__sync_fetch_and_add(&latency_starvation_events, 1);
		return;
	}
	 ...
	 */
	if (scx_bpf_dsq_move_to_local(node_lane_dsq(WOLF_LANE_BULK, node))) {
		if (scx_bpf_dsq_nr_queued(cpu_lane_dsq(WOLF_LANE_LATENCY, cpu)) ||
		    scx_bpf_dsq_nr_queued(node_lane_dsq(WOLF_LANE_LATENCY, node)))
			__sync_fetch_and_add(&latency_starvation_events, 1);
		return;
...
void BPF_STRUCT_OPS(flash_stopping, struct task_struct *p, bool runnable)
{
	...
	if (!rr_sched) {
		...
		if (tctx->lane == WOLF_LANE_BULK)
			__sync_fetch_and_add(&bulk_run_ns_total, slice);
		   ...
		   struct scx_init_task_args *args)
{
	...
	{
		u64 now = scx_bpf_now();
		u64 cgrp_id = args->cgroup->kn->id;
		s32 override_lane;

		tctx->cgrp_id = cgrp_id;
		override_lane = cgrp_lane_override_from_id(cgrp_id);
		tctx->lane_override = (s8)override_lane;
		tctx->lane = override_lane >= 0 ? (u8)override_lane : WOLF_LANE_LATENCY;
		tctx->lane_last_change = now;
		tctx->last_sleep_at = now;
	}
		    ...
		    struct cgroup *from, struct cgroup *to)
{
	...
	tctx->cgrp_id = cgrp_id;
	tctx->lane_override = (s8)override_lane;
	if (override_lane >= 0 && tctx->lane != (u8)override_lane) {
		tctx->lane = (u8)override_lane;
		tctx->lane_last_change = now;
		__sync_fetch_and_add(&lane_switch_total, 1);
	}
}
...
s32 BPF_STRUCT_OPS_SLEEPABLE(flash_init)
{
	...
	/* Create per-CPU DSQs */
	bpf_for(cpu, 0, nr_cpu_ids) {
		err = scx_bpf_create_dsq(cpu_lane_dsq(WOLF_LANE_LATENCY, cpu), __COMPAT_scx_bpf_cpu_node(cpu));
		if (err) {
			scx_bpf_error("failed to create DSQ %d: %d", cpu, err);
			return err;
		}
		err = scx_bpf_create_dsq(cpu_lane_dsq(WOLF_LANE_BULK, cpu), __COMPAT_scx_bpf_cpu_node(cpu));
		if (err) {
			scx_bpf_error("failed to create DSQ %d: %d", cpu, err);
			return err;
		}
	...
	bpf_for(node, 0, __COMPAT_scx_bpf_nr_node_ids()) {
		err = scx_bpf_create_dsq(node_lane_dsq(WOLF_LANE_LATENCY, node), node);
		if (err) {
			scx_bpf_error("failed to create DSQ %d: %d", node, err);
			return err;
		}
		err = scx_bpf_create_dsq(node_lane_dsq(WOLF_LANE_BULK, node), node);
		if (err) {
			scx_bpf_error("failed to create DSQ %d: %d", node, err);
			return err;
		}
