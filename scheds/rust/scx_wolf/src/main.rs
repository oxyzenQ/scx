...
const SCHEDULER_NAME: &str = "scx_wolf";
...
#[command(
    ...
    about = "An aggressive, throughput-oriented scheduler with bounded, predictable behavior.",
    long_about = r#"
scx_wolf is an aggressive, throughput-oriented scheduler that must remain smooth, consistent, and stable.
...
struct Opts {
    ...
    /// Preset which only affects BPF rodata tuning knobs.
    ///
    /// Use `--adaptive-wolf` / `--gaming` for the most explicit preset selection.
    ///
    /// Default: adaptive-wolf
    ...
    #[clap(
        ...
        conflicts_with_all = ["gaming", "preset"],
        long_help = "Alias for `--preset adaptive-wolf`.\n\nThis preset only sets BPF rodata knobs (lane tuning) and does not automatically apply any other flags.\n\nLane knobs:\n- lane_min_dwell_ns = 300ms\n- lane_lat_nvcsw_hi = 6\n- lane_lat_nvcsw_lo = 3\n- lane_bulk_exec_ns = 8ms\n\nSuggested CLI options (manual starting point):\n  scx_wolf --adaptive-wolf -m auto -s 4096 -S 128 -l 4096 -r 32768 -c 128 -I 32"
    )]
    adaptive_wolf: bool,
    ...
    #[clap(
        long,
        action = clap::ArgAction::SetTrue,
        conflicts_with_all = ["adaptive_wolf", "preset"],
        long_help = "Alias for `--preset gaming`.\n\nThis preset only sets BPF rodata knobs (lane tuning) and does not automatically apply any other flags.\n\nLane knobs:\n- lane_min_dwell_ns = 120ms\n- lane_lat_nvcsw_hi = 4\n- lane_lat_nvcsw_lo = 2\n- lane_bulk_exec_ns = 4ms\n\nSuggested CLI options (manual starting point):\n  scx_wolf --gaming -m all -s 2048 -S 64 -l 2048 -L -r 49152 -c 256 -C 85 -I 8 -D -y -p -n --cpufreq"
    )]
    gaming: bool,
    ...
    /// Print a one-shot stats snapshot from a running scheduler and exit.
    #[clap(long, action = clap::ArgAction::SetTrue)]
    dump_stats: bool,
...
impl<'a> Scheduler<'a> {
    fn init(opts: &'a Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        let mut skel = scx_ops_open!(skel_builder, open_object, wolf_ops, open_opts)?;

        skel.struct_ops.wolf_ops_mut().exit_dump_len = opts.exit_dump_len;
        ...
        match opts.preset.unwrap_or(Preset::AdaptiveWolf) {
            Preset::AdaptiveWolf => {
                rodata.lane_min_dwell_ns = 300_000_000;
                rodata.lane_lat_nvcsw_hi = 6;
                rodata.lane_lat_nvcsw_lo = 3;
                rodata.lane_bulk_exec_ns = 8_000_000;
            }
            Preset::Gaming => {
                rodata.lane_min_dwell_ns = 120_000_000;
                rodata.lane_lat_nvcsw_hi = 4;
                rodata.lane_lat_nvcsw_lo = 2;
                rodata.lane_bulk_exec_ns = 4_000_000;
            }
        }
        ...
        // Set scheduler flags.
        skel.struct_ops.wolf_ops_mut().flags = *compat::SCX_OPS_ENQ_EXITING
        ...
        info!(
            "scheduler flags: {:#x}",
            skel.struct_ops.wolf_ops_mut().flags
        );

        // Load the BPF program for validation.
        let mut skel = scx_ops_load!(skel, wolf_ops, uei)?;
        ...
        let struct_ops = Some(scx_ops_attach!(skel, wolf_ops)?);
    ...
    fn get_metrics(&self) -> Metrics {
        ...
        Metrics {
            ...
            lane_switch_total: bss_data.lane_switch_total,
            latency_starvation_events: bss_data.latency_starvation_events,
            bulk_run_ns_total: bss_data.bulk_run_ns_total,
        }
    }
...
fn main() -> Result<()> {
    ...
    if opts.adaptive_wolf {
        opts.preset = Some(Preset::AdaptiveWolf);
    } else if opts.gaming {
        opts.preset = Some(Preset::Gaming);
